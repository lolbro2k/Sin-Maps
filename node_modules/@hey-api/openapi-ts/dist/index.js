...
};`},useData:true};var Fn={compiler:[8,">= 4.3.0"],main:function(e,n,s,t,r){return `export const getRequestBody = (options: ApiRequestOptions): unknown => {
	if (options.body !== undefined) {
		if (options.mediaType?.includes('application/json') || options.mediaType?.includes('+json')) {
			return JSON.stringify(options.body);
		} else if (isString(options.body) || isBlob(options.body) || isFormData(options.body)) {
...
};`},useData:true};var or=()=>{p.registerHelper("camelCase",function(e){return n({case:"camelCase",value:e})}),p.registerHelper("equals",function(e,n,s){return e===n?s.fn(this):s.inverse(this)}),p.registerHelper("ifServicesResponse",function(e$1,n){return e().plugins["@hey-api/sdk"]?.response===e$1?n.fn(this):n.inverse(this)}),p.registerHelper("ifdef",function(...e){let n=e.pop();return e.every(s=>!s)?n.inverse(this):n.fn(this)}),p.registerHelper("notEquals",function(e,n,s){return e!==n?s.fn(this):s.inverse(this)}),p.registerHelper("transformServiceName",function(e$1){return y({config:e(),name:e$1})});},ct=()=>{or();let e={client:p.template(bn),core:{apiError:p.template(kn),apiRequestOptions:p.template(wn),apiResult:p.template(En),baseHttpRequest:p.template(jn),cancelablePromise:p.template(Nn),httpRequest:p.template(tt),request:p.template(ot),settings:p.template(rt)}};return p.registerPartial("functions/base64",p.template(Vn)),p.registerPartial("functions/catchErrorCodes",p.template(Gn)),p.registerPartial("functions/getFormData",p.template(zn)),p.registerPartial("functions/getQueryString",p.template(Kn)),p.registerPartial("functions/getUrl",p.template(Xn)),p.registerPartial("functions/isBlob",p.template(Qn)),p.registerPartial("functions/isFormData",p.template(Jn)),p.registerPartial("functions/isString",p.template(Yn)),p.registerPartial("functions/isStringWithValue",p.template(Zn)),p.registerPartial("functions/isSuccess",p.template(et)),p.registerPartial("functions/resolve",p.template(nt)),p.registerPartial("fetch/getHeaders",p.template(_n)),p.registerPartial("fetch/getRequestBody",p.template(Fn)),p.registerPartial("fetch/getResponseBody",p.template(Mn)),p.registerPartial("fetch/getResponseHeader",p.template(Wn)),p.registerPartial("fetch/request",p.template(Ln)),p.registerPartial("fetch/sendRequest",p.template(Un)),p.registerPartial("xhr/getHeaders",p.template(st)),p.registerPartial("xhr/getRequestBody",p.template(it)),p.registerPartial("xhr/getResponseBody",p.template(at)),p.registerPartial("xhr/getResponseHeader",p.template(lt)),p.registerPartial("xhr/request",p.template(ut)),p.registerPartial("xhr/sendRequest",p.template(pt)),p.registerPartial("axios/getHeaders",p.template(Sn)),p.registerPartial("axios/getRequestBody",p.template(Hn)),p.registerPartial("axios/getResponseBody",p.template(Dn)),p.registerPartial("axios/getResponseHeader",p.template(In)),p.registerPartial("axios/request",p.template(Bn)),p.registerPartial("axios/sendRequest",p.template($n)),p.registerPartial("angular/getHeaders",p.template(xn)),p.registerPartial("angular/getRequestBody",p.template(Cn)),p.registerPartial("angular/getResponseBody",p.template(qn)),p.registerPartial("angular/getResponseHeader",p.template(An)),p.registerPartial("angular/request",p.template(Tn)),p.registerPartial("angular/sendRequest",p.template(On)),e};var sr={stringCase:n};P.enabled=ar().hasBasic;var Ti=async e=>{let n=typeof e=="function"?await e():e,s$1=[];try{f.start("createClient"),f.start("config");for(let i of await G(n))if(s$1.push(i.config),i.errors.length)throw i.errors[0];f.end("config"),f.start("handlebars");let t=ct();f.end("handlebars");let o=(await Promise.all(s$1.map(i=>re({config:i,templates:t})))).filter(i=>!!i);f.end("createClient");let a=s$1[0];return a&&a.logs.level==="debug"&&new M({totalMark:"createClient"}).report({marks:["config","openapi","handlebars","parser","generator","postprocess"]}),o}catch(t$1){let r=s$1[0],o=r?r.dryRun:n?.dryRun,a=r?.logs??C(n),i;throw a.level!=="silent"&&a.file&&!o&&(i=s(t$1,a.path??"")),a.level!=="silent"&&(u({error:t$1,logPath:i}),await v()&&await t(t$1)),t$1}},Oi=async e=>typeof e=="function"?await e():e;export{Ti as createClient,Oi as defineConfig,sr as utils};//# sourceMappingURL=index.js.map
